...
def req_2(catalog, min_price, max_price):
    ...
    if lt.size(lista_filtrada) == 0:
        return {
            ...
            "message": "No hay pedidos que cumplan el filtro",
            "total_productos": 0
        }
    else:
        descuento_total = 0
        ...
        for i in range(1,lt.size(lista_filtrada)):
            ...
            if actual["Amount"] > max_order["Amount"] or (actual["Amount"] == max_order["Amount"] and actual["Price_per_Box"] < max_order["Price_per_Box"]):
                max_order = actual
    dicreciente={"Producto": recent["Product"], 
                 "Pais": recent["Country"],
                 "Canal": recent["Channel"],
                 "Fecha": recent["Order_Date"],
                 "Amount": recent["Amount"],
                 "Price_per_box": recent["Price_per_Box"]}
    dicminimo={"Producto": min_order["Product"],
               "Pais": min_order["Country"],
               "Canal": min_order["Channel"],
               "Fecha": min_order["Order_Date"],
               "Amount": min_order["Amount"],
               "Price_per_box": min_order["Price_per_Box"]}
    dicmaximo={"Producto": max_order["Product"],
               "Pais": max_order["Country"],
               "Canal": max_order["Channel"],
               "Fecha": max_order["Order_Date"],
               "Amount": max_order["Amount"],
